...
from src.models.backbones.bricks import BuildNormalization, BuildDropout, FFN, PatchEmbed, PatchMerging
...
class SwinTransformer(nn.Module):
    def __init__(self, structure_type, pretrain_img_size=224, in_channels=3, embed_dims=96, patch_size=4, window_size=7, mlp_ratio=4, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 strides=[4, 2, 2, 2], out_indices=[0, 1, 2, 3], qkv_bias=True, qk_scale=None, patch_norm=True, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.3,
                 use_abs_pos_embed=False, act_cfg={'type': 'GELU'}, norm_cfg={'type': 'LayerNorm'}, use_checkpoint=False, pretrained=False, pretrained_model_path=''):
    ...
    def loadpretrainedweights(self, structure_type='swin_tiny_patch4_window7_224', pretrained_model_path=''):
        ...
        if pretrained_model_path and os.path.exists(pretrained_model_path):
            ...
            checkpoint = model_zoo.load_url(DEFAULT_MODEL_URLS[structure_type],
                        model_dir=pretrained_model_path or None,
